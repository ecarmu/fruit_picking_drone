...
while True:
    ...
    if do_inference:
        ...
        for (x1, y1, x2, y2), original_tid in zip(boxes, ids):
            original_tid = int(original_tid)
            if original_tid not in id_remap:
                id_remap[original_tid] = next_sequential_id
                next_sequential_id += 1
            tid = id_remap[original_tid]
